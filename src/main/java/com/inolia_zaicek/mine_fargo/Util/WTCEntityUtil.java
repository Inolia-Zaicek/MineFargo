...
public class WTCEntityUtil {
    ...
    public static void moveTowardsTarget(Entity entity) {
        if (entity != null) {
            ...
            List<LivingEntity> entities = entity.level().getEntitiesOfClass(LivingEntity.class,
                    entity.getBoundingBox().inflate((double) MyGoConfig.track_range.get()),
                    (target) -> targetPredicate.test(target) && target != owner && target.isAlive() && (owner == null || !target.isAlliedTo(owner)) && (!entity.level().isClientSide() || target != Minecraft.getInstance().player));
            if (!entities.isEmpty()) {
    ...
    public static void moveTowardsTargetWithSpeed(Entity entity, double speed) {
        ...
        var entities = entity.level().getEntitiesOfClass(LivingEntity.class,
                entity.getBoundingBox().inflate((double) MyGoConfig.track_range.get()),
                (target) -> finalTargetPredicate.test(target) && target != null && target.isAlive());
