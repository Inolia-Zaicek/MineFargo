...
public class GoetyUseEvent {
    ...
    public static void UseItemEvent(LivingEntityUseItemEvent.Finish event) {
        if (ModList.get().isLoaded("goety")) {
            //药水
            if (MyGoUtil.hasGoetyItem(event.getEntity(), BrewSoulStone.get()) && event.getItem().getItem() instanceof BrewItem
                    && new Random().nextInt(100) < MyGoConfig.brew_soul_stone_chance_a.get()) {
                event.setResultStack(event.getItem());
            } else if (MyGoUtil.hasGoetyItem(event.getEntity(), BrewSoulStone.get()) && event.getItem().getItem() instanceof PotionItem
